...
// Client A Client is an authenticated Zabbix JSON-RPC API client. It must be
// initialized and connected with NewClient.
type Client struct {
...
func Login(username string, password string) Option {
	return func(s *Client) error {
		return s.login(username, password)
	}
}
func PlaceToken(token string) Option {
	return func(s *Client) error {
		s.Token = token
		return nil
	}
...
// NewClient Create a zabbix certification
func NewClient(url string, options ...Option) (session *Client, err error) {
	// create session
...
func (c *Client) Get(method string, params interface{}, v interface{}) error {
